...
pub struct Handle {
    ...
    pub accumulator_task: JoinHandle<()>,
}
...
impl StateAccumulator {
    ...
    pub async fn start(mut self) -> Handle {
        let handle = spawn_monitored_task!(async move { self.run() });
        tokio::spawn(accumulator_task);

        Handle {
            sender: self.sender,
            store: self.store.clone(),
            accumulator_task: handle,
        }
    }

    async fn run(&mut self) {
        let (last_checkpoint, epoch_id) = self.gen_checkpoint_state_hashes().await;
        self.accumulate(last_checkpoint, epoch_id)
            .await
    ...
    async fn gen_checkpoint_state_hashes(&mut self) -> (CheckpointSequenceNumber, EpochId) {
